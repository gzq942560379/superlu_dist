...
while (j < nub && iperm_u[j] <= k0 + num_look_aheads)
{
    ...
    // printf("j jb nub iperm_u[j] k0 num_look_aheads : %d %d %d %d %d %d\n", j, jb, nub, iperm_u[j], k0, num_look_aheads);
    ...
    /* Each thread starts searching from first block.
                          This records the moving search target.           */
    /* point to the start of index[] in supernode L(:,k) */

    lptr = lptr0; 
    luptr = luptr0;

    // init L list and L_all
    int all_row = 0;
...
#endif
        LBlock_info[lb].ib = lsub[lptr];
        LBlock_info[lb].lptr = lsub[lptr + 1];
    ...
    /* calling gemm */
#if defined(USE_SW) && defined(OPT_gather_avoid)
    zgemm_("N", "N", &L_all.row, &U.col, &U.row, &alpha,
                L_all.val, &L_all.ld,
                U.val, &U.ld, &beta, tempv, &all_row, 1, 1);
#else
    zgemm_("N", "N", &all_row, &ncols, &ldu, &alpha,
                &lusup[luptr0 + (knsupc - ldu) * nsupr], &nsupr,
                tempu, &ldu, &beta, tempv, &all_row, 1, 1);
#endif 
...
#if defined(USE_SW) && defined(OPT_scatter_index_compress)
            zscatter_l_sw (ib, ljb, nsupc, iukp, xsup, klst, all_row, lptr,
    ...
    // printf("panel factorization look_ahead[kk] k0 kcol mycol: %d %d %d %d\n",look_ahead[kk],k0,kcol,mycol);
