...
#include <stdbool.h> 
#include <math.h>
#include "opt/superlu_zdefs_ex.h"
...
#define PZGSTRF2 lpanelfact_trsm
...
// #include "zscatter.c"
// #include "opt/doublecomplex/zscatter_fugaku.c"
...
#pragma omp parallel default(shared)
    ...
    // show_supernode_size(n, Glu_persist, grid);
    // show_L_structure(n, LUstruct, grid);
    // show_U_structure(n, LUstruct, grid);
    // check_symmetry(n, LUstruct, grid);
...
// 	    TIC (t1);
// #endif
//             // printf("[%d] Irecv U(%d,:) from %d (%d, %d)\n", iam, k, PNUM(krow, mycol, grid), krow, mycol);
//             // fflush(stdout);
...
// #endif
//         }
        bcast_t* ubcast_0 = &Llu->UB[0];
        recv_reqs_u[0][0] = MPI_REQUEST_NULL;
        recv_reqs_u[0][1] = MPI_REQUEST_NULL;
        if(ubcast_0->in){
            scp = &grid->cscp;  /* The scope of process column. */
            Usub_buf = Llu->Usub_buf_2[0];
            Uval_buf = Llu->Uval_buf_2[0];
            int recvfrom = ubcast_0->recvfrom;
            int recvfrom_pr = recvfrom / grid->npcol;
#if ( PROFlevel>=1 )
	    TIC (t1);
#endif
            // printf("[%d] Irecv U(%d,:) from %d (%d, %d)\n", iam, k, PNUM(krow, mycol, grid), krow, mycol);
            // fflush(stdout);
            MPI_Irecv (Usub_buf, Llu->bufmax[2], mpi_int_t, recvfrom_pr,
                       SLU_MPI_TAG (2, 0) /* 2%tag_ub */ ,
                       scp->comm, &recv_reqs_u[0][0]);
            MPI_Irecv (Uval_buf, Llu->bufmax[3], SuperLU_MPI_DOUBLE_COMPLEX, recvfrom_pr,
                       SLU_MPI_TAG (3, 0) /* 3%tag_ub */ ,
                       scp->comm, &recv_reqs_u[0][1]);
#if ( PROFlevel>=1 )
	    TOC (t2, t1);
...
// 		    TIC (t1);
// #endif
//                     // printf("[%d] Irecv U(%d,:) from %d (%d, %d)\n", iam, kk, PNUM(krow, mycol, grid), krow, mycol);
//                     // fflush(stdout);
//                     MPI_Irecv (Usub_buf, Llu->bufmax[2], mpi_int_t, krow,
...
// #endif
//                 }
                recv_reqs_u[look_id][0] = MPI_REQUEST_NULL;
                recv_reqs_u[look_id][1] = MPI_REQUEST_NULL;
                if(ubcast_kk->in){
                    scp = &grid->cscp;  /* The scope of process column. */
                    Usub_buf = Llu->Usub_buf_2[look_id];
                    Uval_buf = Llu->Uval_buf_2[look_id];
		    ...
		    TIC (t1);
#endif
                    int recvfrom = ubcast_kk->recvfrom;
                    int recvfrom_pr = recvfrom / grid->npcol;
                    // printf("[%d] Irecv U(%d,:) from %d (%d, %d)\n", iam, kk, PNUM(krow, mycol, grid), krow, mycol);
                    // fflush(stdout);
                    MPI_Irecv (Usub_buf, Llu->bufmax[2], mpi_int_t, recvfrom_pr,
...
#endif
			{
#if defined(USE_OPT) && defined(OPT_pzgstrs2)
                            pzgstrs2_fugaku (kk0, kk, Glu_persist, grid, Llu,
                                        Ublock_info, stat);
#else
...
//                         }       /* if ToSendD ... */
                        ...
                        if(iam == ubcast_kk->root){
                            for(pr = 0; pr < Pr; ++pr) {
                                send_reqs_u[look_id][pr] = MPI_REQUEST_NULL;
                                send_reqs_u[look_id][pr + Pr] = MPI_REQUEST_NULL;
                            }
                            for(int ii = 0; ii < ubcast_kk->sendto_size; ++ii){
                                int sendto = ubcast_kk->sendto[ii];
                                int sendto_pr = sendto / grid->npcol;
#if ( PROFlevel>=1 )
                                TIC (t1);
#endif
                                MPI_Isend (usub, msgcnt[2], mpi_int_t, sendto_pr,
                                    SLU_MPI_TAG (2, kk0), /* (4*kk0+2)%tag_ub */
                                    scp->comm, &send_reqs_u[look_id][pi]);
                                MPI_Isend (uval, msgcnt[3], SuperLU_MPI_DOUBLE_COMPLEX,
                                    sendto_pr, SLU_MPI_TAG (3, kk0), /* (4*kk0+3)%tag_ub */
                                    scp->comm, &send_reqs_u[look_id][pi + Pr]);
#if ( PROFlevel>=1 )
                                TOC (t2, t1);
                                stat->utime[COMM] += t2;
...
/* #pragma omp parallel */ /* Sherry -- parallel done inside pzgstrs2 */
                ...
                {
#if defined(USE_OPT) && defined(OPT_pzgstrs2)
                    pzgstrs2_fugaku (k0, k, Glu_persist, grid, Llu, 
		                    Ublock_info, stat);
#else
...
//                 } /* if ToSendD ... */
                ...
                if(iam == ubcast_k->root){
                    for(int ii = 0; ii < ubcast_k->sendto_size; ++ii){
                        int sendto = ubcast_k->sendto[ii];
                        int sendto_pr = sendto / grid->npcol;
#if ( PROFlevel>=1 )
                        TIC (t1);
#endif
                        MPI_Send (usub, msgcnt[2], mpi_int_t, sendto_pr,
                                    SLU_MPI_TAG (2, k0), /* (4*k0+2)%tag_ub */
                                    scp->comm);
                        MPI_Send (uval, msgcnt[3], SuperLU_MPI_DOUBLE_COMPLEX, sendto_pr,
                                    SLU_MPI_TAG (3, k0), /* (4*k0+3)%tag_ub */
                                    scp->comm);
#if ( PROFlevel>=1 )
                        TOC (t2, t1);
...
#endif
                    for (pi = 0; pi < Pr; ++pi) {
                        if (pi != myrow && send_reqs_u[look_id][pi] != MPI_REQUEST_NULL
                                && send_reqs_u[look_id][pi + Pr] != MPI_REQUEST_NULL) {
                            MPI_Wait (&send_reqs_u[look_id][pi], &status);
                            MPI_Wait (&send_reqs_u[look_id][pi + Pr], &status);
                        }
                    }
...
//                 msgcnt[2] = 0;
            ...
            if (ubcast_k->in) { /* Recv block row U(k,:). */
                assert(recv_reqs_u[look_id][0] != MPI_REQUEST_NULL && recv_reqs_u[look_id][1] != MPI_REQUEST_NULL);
#if ( PROFlevel>=1 )
                TIC (t1);
#endif
                MPI_Wait (&recv_reqs_u[look_id][0], &status);
...
#ifdef USE_OPT
#include "opt/zlook_ahead_update_fugaku.c"
#endif
#if !defined(USE_OPT) 
...
#ifdef USE_OPT
#include "opt/zSchCompUdt-2Ddynamic-fugaku.c"
#endif
#if !defined(GPU_ACC) && !defined(USE_OPT)
