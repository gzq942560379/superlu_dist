            ...
            gridinfo_t * grid)
{
    ...
    int_t* segment_ptr = SUPERLU_MALLOC((segment_count+1) * sizeof(int_t));
    int_t* segment_offset = SUPERLU_MALLOC(segment_count * sizeof(int_t));
    segment_ptr[0] = 0;
    ...
    for (int jj_ptr = 0; jj_ptr < non_zero_col_num; ++jj_ptr) {
        ...
        for(int ptr = 0; ptr < segment_count; ++ptr){
            ...
            int i_end = segment_ptr[ptr+1];
            int offset = segment_offset[ptr];
            doublecomplex *NZVAL = nzval_cur + offset;
            for(int i = i_start; i < i_end; i++){
    ...
    SUPERLU_FREE(segment_offset);
    SUPERLU_FREE(segment_ptr);
} /* zscatter_l */
            ...
            gridinfo_t * grid)
{
    ...
    for (int jj_ptr = 0; jj_ptr < non_zero_col_num; ++jj_ptr) {
        ...
        doublecomplex *tempv_cur = tempv + nbrow * jj_ptr;
        for(int ptr = 0; ptr < segment_count; ++ptr){
            int i_start = segment_ptr[ptr];
            int i_end = segment_ptr[ptr+1];
            int offset = segment_offset[ptr];
            int rel = offset - fnz;
            doublecomplex *UCOL = &ucol_cur[rel];
            for(int i = i_start; i < i_end; i++){
