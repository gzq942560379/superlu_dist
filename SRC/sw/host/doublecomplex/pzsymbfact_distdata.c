...
/* Count the nnzs per block column */
	for (lb = 0; lb < nub; ++lb) {
		...
		for (ub = 0; ub < Urbs[lb]; ++ub) {
			...
			t = SuperLU_timer_();
#endif
    // build global dependent graph
    egraph_t *graph, *graph_r;
    graph = SUPERLU_MALLOC(sizeof(egraph_t));
    graph_r = SUPERLU_MALLOC(sizeof(egraph_t));
    egraph_global_build(graph, n, Glu_persist, Lrowind_bc_ptr, grid);
#if ( PROFlevel>=1 )
    t = SuperLU_timer_() - t;
    if ( !iam) printf("Construct Graph : %.2f\t\n", t);
#endif  

#if ( PROFlevel>=1 )
			t = SuperLU_timer_();
#endif
    egraph_global_reverse(graph_r, graph);
    Llu->graph = graph;
    Llu->graph_r = graph_r;
#if ( PROFlevel>=1 )
    t = SuperLU_timer_() - t;
    if ( !iam) printf("Construct Reversed Graph : %.2f\t\n", t);
#endif  
#if ( PROFlevel>=1 )
			t = SuperLU_timer_();
#endif
    bcast_build_LB(&Llu->LB, graph, nsupers, grid);
#if ( PROFlevel>=1 )
    t = SuperLU_timer_() - t;
    if ( !iam) printf("Construct LBcast : %.2f\t\n", t);
#endif  
#if ( PROFlevel>=1 )
			t = SuperLU_timer_();
#endif
    bcast_build_UB(&Llu->UB, graph, nsupers, grid);
#if ( PROFlevel>=1 )
    t = SuperLU_timer_() - t;
    if ( !iam) printf("Construct UBcast : %.2f\t\n", t);
#endif  
