       ...
       zLUstruct_t * LUstruct, gridinfo_t * grid, SuperLUStat_t * stat, int *info)
{
    ...
    *info = 0;
...
#if ( PRNTlevel>=1 )
    if(!iam) {
       printf("Enter pzgstrf in sw/host/pzgstrf.c\n");
       fflush(stdout);
    }
#endif
...
#if ( PRNTlevel>=1 )
    if(!iam){
        printf("options->num_lookaheads : %d\n", options->num_lookaheads);
        fflush(stdout);
    }
#endif
    if (Pr * Pc > 1) {
    ...
    for (k0 = 0; k0 < nsupers; ++k0) {
        // if(!iam){
        //     printf("processing %d in %d supernode \n", k0, nsupers);
        //     fflush(stdout);
        // }
...
#include "sw/host/zlook_ahead_update.c"
...
#include "sw/host/zSchCompUdt-2Ddynamic.c"
// #include "zSchCompUdt-2Ddynamic.c"
    ...
    float flopcnt = 0;
    MPI_Reduce(&stat->ops[FACT], &flopcnt, 1, MPI_FLOAT, MPI_SUM, 0, grid->comm);
    double allflops;
    ...
    printf("Total FLOPs for factorization : %8.4f\n", flopcnt);
    printf("Total TFLOPS for factorization : %8.4f\n", flopcnt * 1e-12 / pxgstrfTimer);
    }
#endif
...
#if ( PRNTlevel>=1 )
    if(!iam) {
       printf("Exit pzgstrf in sw/host/pzgstrf.c\n");
       fflush(stdout);
    }
#endif
