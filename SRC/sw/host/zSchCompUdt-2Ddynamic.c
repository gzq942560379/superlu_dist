	...
	*/
	...
	if ( nbrow > 0 ) { /* L(:,k) is not empty */
		...
		* After this, full_u_cols is the number of nonzero columns
		...
		// for(int c = 0; c < U_all.col; ++c){
		// 	for(int r = 0; r < U_all.row; ++r){
		// 		float tmp_real = U_all.val[r + c * U_all.ld].r;
		// 		float tmp_imag = U_all.val[r + c * U_all.ld].i;
		// 		U_all.val[r + c * U_all.ld].r = tmp_real;
		// 		U_all.val[r + c * U_all.ld].i = tmp_imag;
		// 	}
		// }
...
// #endif
...
#ifdef OPT_scatter_index_compress
    	// athread_spawn(lookAhead_scatter_1_index_compress, &lookAhead_scatter_param);
    	athread_spawn(lookAhead_scatter_2_index_compress_dma, &lookAhead_scatter_param);
#else
		...
		/* jj0 starts after look-ahead window. */
			...
			if ( ib < jb ) {
#if defined(OPT_scatter_index_compress)
				zscatter_u_opt (
					ib, jb,
					nsupc, iukp, xsup,
    ...
    if ( Rnbrow>0 && ldu>0 ) { /* There are still blocks remaining ... */
		...
		if( iam == 0 ){
			printf("Memory access bandwidth utilization for read is (by swperf) : %lf\n", read_ratio);
			printf("Memory access bandwidth utilization for write is (by swperf) : %lf\n", write_ratio); 
		}
#endif 
...
#ifdef OPT_scatter_index_compress
    	// CRTS_athread_spawn(remain_scatter_1_index_compress, &remain_scatter_param);
    	CRTS_athread_spawn(remain_scatter_2_index_compress_dma, &remain_scatter_param);
#else
    	CRTS_athread_spawn(remain_scatter_0_naive, &remain_scatter_param);
#endif
		CRTS_athread_join();
#else
