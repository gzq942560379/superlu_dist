...
#define MAX_SUPERNODE_SIZE 128

__thread_local int segment_ptr[MAX_SUPERNODE_SIZE + 1];
__thread_local int segment_offset[MAX_SUPERNODE_SIZE];

__thread_local int indirect[MAX_SUPERNODE_SIZE];
__thread_local int indirect2[MAX_SUPERNODE_SIZE];

__thread_local doublecomplex * non_zero_ucols[MAX_SUPERNODE_SIZE];
__thread_local int fnzs[MAX_SUPERNODE_SIZE];
__thread_local doublecomplex* nzvals[MAX_SUPERNODE_SIZE];

__thread_local double SRC[MAX_SUPERNODE_SIZE * 4];
__thread_local double TARGET[MAX_SUPERNODE_SIZE * 4];
        ...
        for(int i = local_i_start;i < local_i_end;i++){
			...
			if ( ib < jb ) {
                ...
                */
                ...
                double* SRC_next = &SRC[MAX_SUPERNODE_SIZE * 2 * source_flag_next];
                ...
                for (; jj_ptr_next <= non_zero_col_num; ++jj_ptr_next) {
                    ...
                    double* SRC_cur = &SRC[MAX_SUPERNODE_SIZE * 2 * source_flag_cur];
                    ...
                    if(jj_ptr_next < non_zero_col_num){
                        ...
                        SRC_next = &SRC[MAX_SUPERNODE_SIZE * 2 * source_flag_next];
                        get_source_reply[source_flag_next] = 0;
                        athread_dma_iget(SRC_next, tempv + nbrow * jj_ptr_next, 2 * temp_nbrow * sizeof(double), &get_source_reply[source_flag_next]);
                    }
                    // wait currnet SRC
                    ...
                    // process current SRC
                    {
                        ...
                        double* TARGET_next = &TARGET[MAX_SUPERNODE_SIZE * 2 * target_flag_next];
                        ...
                        for(; ptr_next <= segment_count; ++ptr_next){
                            ...
                            double* TARGET_cur  = &TARGET[MAX_SUPERNODE_SIZE * 2 * target_flag_cur];
                            ...
                            if(ptr_next < segment_count){
                                ...
                                TARGET_next = &TARGET[MAX_SUPERNODE_SIZE * 2 * target_flag_next];
                                get_target_reply[target_flag_next] = 0;
                                athread_dma_iget(TARGET_next, &ucol_cur[rel_next + i_start_next], i_len_next * sizeof(double), &get_target_reply[target_flag_next]);
                            }
